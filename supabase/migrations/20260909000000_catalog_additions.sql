-- Adds four new one-time catalog items (branding/logo, accessibility audit,
-- ads campaign setup, social media setup) to the existing production-task
-- catalog. Copywriting already maps to the 'content' key -- it only needed a
-- clickable proposal preset (added client-side), no catalog change here.
...
as $$
  select case lower(trim(both from coalesce(p_label, '')))
    ...
    when 'branding / logo design' then 'branding'
    when 'branding' then 'branding'
    when 'logo design' then 'branding'
    when 'accessibility audit' then 'accessibility'
    when 'ada/wcag audit' then 'accessibility'
    when 'ads campaign setup' then 'ads_setup'
    when 'google/meta ads setup' then 'ads_setup'
    when 'social media setup' then 'social_setup'
    when 'social media setup & content calendar' then 'social_setup'
...
as $$
  select case p_key
    ...
    when 'branding' then 'design'
    when 'accessibility' then 'feature'
    when 'ads_setup' then 'feature'
    when 'social_setup' then 'feature'
    else null
  end;
$$;
...
as $$
  select case p_key
    ...
    when 'branding' then 'Branding / Logo design'
    when 'accessibility' then 'Accessibility audit'
    when 'ads_setup' then 'Ads campaign setup'
    when 'social_setup' then 'Social media setup'
    else null
  end;
$$;
...
begin
  ...
  if 'branding' = any (keys) then
    select e.p_pos, e.p_inserted into pos, inserted
    from public.enqueue_production_task(
      v_project_id, design_id,
      'Design brand identity / logo',
      'Create the logo and brand identity included in the accepted proposal.',
      pos, inserted
    ) e;
  end if;
  ...
  if 'accessibility' = any (keys) then
    select e.p_pos, e.p_inserted into pos, inserted
    from public.enqueue_production_task(
      v_project_id, review_id,
      'Accessibility audit (ADA/WCAG)',
      'Audit the staging website against WCAG 2.1 AA and document required fixes.',
      pos, inserted
    ) e;
  end if;
  ...
  if 'ads_setup' = any (keys) then
    select e.p_pos, e.p_inserted into pos, inserted
    from public.enqueue_production_task(
      v_project_id, launch_id,
      'Set up ad campaign',
      'Set up the Google/Meta ad campaign included in the accepted proposal.',
      pos, inserted
    ) e;
  end if;
  if 'social_setup' = any (keys) then
    select e.p_pos, e.p_inserted into pos, inserted
    from public.enqueue_production_task(
      v_project_id, launch_id,
      'Set up social media & content calendar',
      'Set up social profiles and an initial content calendar included in the accepted proposal.',
      pos, inserted
    ) e;
  end if;
