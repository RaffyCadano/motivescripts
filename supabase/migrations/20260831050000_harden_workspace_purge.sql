-- Harden Danger Zone so a full commercial-workflow test workspace can be reset.
-- Paid invoices, accepted proposals/contracts, portal Auth users, scope briefs,
-- and Storage objects are included. Team accounts and Settings stay.
...
begin
  if tg_op = 'DELETE' then
    if public.document_rpc_active() or public.workspace_purge_active() then
      return old;
    end if;
...
create or replace function public.purge_workspace_storage(p_include_contracts boolean)
returns boolean
language plpgsql
security definer
    ...
    and (
      ...
      or (
        ...
        and (
          name like 'contracts/%'
          or name in (
            select client_signed_copy_path
            from public.contract_revisions
            where client_signed_copy_path is not null
          )
        )
      )
    );
...
create or replace function public.purge_client_portal_auth()
returns integer
language plpgsql
security definer
...
begin
  ...
  where u.id is distinct from auth.uid()
    and exists (
      ...
      where p.id = u.id
        and p.role = 'client'
        and not exists (select 1 from public.staff_profiles s where s.user_id = p.id)
    );
...
comment on function public.purge_workspace(text, text) is
  'Admin-only test workspace reset. Requires an exact confirmation phrase. Deletes progressed client/project records including paid invoices and portal Auth users. Does not delete team accounts, Settings, or Stripe Dashboard objects.';

revoke all on function public.purge_workspace_storage(boolean) from public, anon, authenticated;
revoke all on function public.purge_client_portal_auth() from public, anon, authenticated;
