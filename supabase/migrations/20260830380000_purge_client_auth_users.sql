-- Danger Zone: delete client portal Auth users after workspace records are gone.
-- Admin and staff Auth accounts stay. The signed-in administrator is never deleted.
...
comment on function public.purge_workspace(text, text) is
  'Admin-only workspace wipe. Clients/agency scopes also delete client portal Auth users. Team accounts and Settings stay.';
