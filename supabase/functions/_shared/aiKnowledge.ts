 ...
 */
export const AI_KNOWLEDGE_VERSION = "2026-11-04.1";
...
- Hosting: free for the first 30 days after the website goes live, for every package. After that the website is paused unless the client has an active Website Care plan (hosting is included from the Essential tier). The client is emailed a week and a day before the free period ends, and is told when it is paused. Choosing a plan from the client portal keeps the site online or brings a paused site back. During the free 30 days the client can also send free Website Care requests (small updates and fixes). Hosting setup for a new project is a separate line item quoted in the proposal.
- Domain name: included for the first year with the Growth package and with Custom (scoped in the proposal); an add-on with the Website package. Business email account: included with Custom (scoped in the proposal); an add-on with the Website and Growth packages. Add-ons are quoted as separate line items in the proposal; do not quote prices for them. Hosting setup, domain registration, and business email are otherwise not automatically part of the starting price.
- Every package includes: approving the design and the finished site before launch, and a client portal with project approvals, feedback, messages, proposals, contracts, and invoices that can be paid online. Growth and Custom also include the project files library in the portal. A live website status appears in the portal once uptime checks are running for the site, which comes with a Website Care plan; it is not part of the Website package. The number of revision rounds is set in the project proposal; never state a fixed number.
- Website Care is optional, separate from the website project, and never included automatically. It is the only ongoing monthly plan MotiveScripts offers -- hosting and SEO are not sold as separate plans, they're included in Website Care starting at the Essential and Pro tiers respectively. It is billed monthly and renews automatically until canceled. Published STARTING price: ${CARE_STARTING_PRICE}/month (the Essential tier; higher tiers cost more and add more). This is a starting price. Website Care is only available once the client's website has launched: after that, the client can choose a tier themselves from their client portal at the listed monthly price and confirm it on a secure checkout page before anything is charged. A different or custom plan can be arranged by asking the team. Clients can cancel a plan themselves from their client portal at any time: an active plan then stays active until the end of the period already paid for and they are not charged again. To change a plan (rather than cancel it), they contact the team. Hosting setup for a new website project is a separate item quoted in the proposal, distinct from ongoing hosting under Website Care. Separately, right after a website launches, MotiveScripts gives a 30-day grace period during which the client can still submit Website Care support requests (small updates, fixes) even without an active plan yet -- after that window, a plan is required to keep sending requests.
