...
import { teamNotificationEvents } from "@/data/notificationPreferences";
...
export function TeamProfile() {
  ...
  const notificationEvents = teamNotificationEvents({
    canFiles: hasPermission(profile, "files.view"),
    canMessages: hasPermission(profile, "messages.view") && isProductionCommunicator(profile),
  });
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-[var(--admin-radius)] border border-[var(--admin-line)] bg-[var(--admin-card)] p-5 md:p-6">
        ...
        <div className="mt-4">
          {notificationEvents.length === 0 ? (
            <p className="text-sm text-[var(--admin-muted)]">
              There are no optional notifications for your role. Task, deadline, and payroll notifications are always
              delivered.
            </p>
          ) : (
        ...
        <div className="mt-6 border-t border-[var(--admin-line)] pt-5">
          ...
          <div className="mt-4">
            <EmailAlertPreferences categories={["client_activity", "site_alerts", "team_work"]} />
          </div>
        </div>
      </section>
