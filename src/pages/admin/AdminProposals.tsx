...
import { fetchProposalSummaries, type ProposalSummary } from "@/data/documentsRepository";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="font-heading text-[1.65rem] font-semibold tracking-tight md:text-3xl">Proposals</h1>
          <p className="mt-1 text-sm text-[var(--admin-muted)]">Scope, investment, and terms for client review.</p>
        </div>
        <button
          type="button"
      ...
      <section aria-label="Proposal snapshot" className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
        {[
          ["Draft", counts.draft],
          ["Sent", counts.sent],
      ...
      {loading ? (
        ...
        <div className="rounded-[var(--admin-radius)] border border-dashed border-[var(--admin-line)] bg-[var(--admin-card)] px-5 py-10">
          ...
          <p className="mt-1 text-sm text-[var(--admin-muted)]">Create a proposal after the client record exists.</p>
        </div>
      ) : (
        <>
              ...
              <tbody>
                {visible.map((row) => (
                  <tr key={row.id} className="border-b border-[var(--admin-line)] last:border-b-0 hover:bg-[var(--admin-bg)]">
                    ...
                    <td className="px-5 py-3.5">{row.title || "Untitled"}</td>
                    ...
                    <td className="px-5 py-3.5">
                      <Link to={`/admin/proposals/${row.id}`} className="font-heading text-[12px] font-semibold text-[var(--admin-blue)] hover:underline">
                        View
                      </Link>
                    </td>
                  </tr>
              ...
              <li key={row.id} className="rounded-[var(--admin-radius)] border border-[var(--admin-line)] bg-[var(--admin-card)] p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm">{row.title || "Untitled"}</p>
                    <p className="mt-1 text-[12px] text-[var(--admin-muted)]">
                      {clients.find((item) => item.id === row.clientId)?.businessName ?? "—"}
                    </p>
                  </div>
                  <DocumentStatusBadge status={row.effectiveStatus} />
                </div>
                <p className="mt-3 font-heading text-sm font-semibold">{formatUsdFromCents(row.investmentCents)}</p>
                <Link to={`/admin/proposals/${row.id}`} className="mt-3 inline-flex font-heading text-[12px] font-semibold text-[var(--admin-blue)]">
                  View
                </Link>
              </li>
            ))}
