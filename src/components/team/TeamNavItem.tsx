...
import type { TeamNavItem as TeamNavItemData } from "@/data/teamNav";
import { cn } from "@/lib/cn";
import { useMessaging } from "@/providers/MessagingProvider";

type TeamNavItemProps = {
  item: TeamNavItemData;
  collapsed: boolean;
  onNavigate?: () => void;
};

export function TeamNavItem({ item, collapsed, onNavigate }: TeamNavItemProps) {
  ...
  // Several developer nav items point at #anchors on the same /team/dashboard route, so
  // React Router's own pathname-only isActive would mark all of them active together. An
  // item that targets a hash is only active when that exact hash is current; an item with
  // no hash (e.g. Overview) is only active when the URL has no hash at all.
          ...
          <>
            <span className="relative shrink-0">
              <Icon
                ...
                className={cn(
                  "shrink-0",
                  isActive ? "text-[var(--admin-blue)]" : "text-[var(--admin-muted)] group-hover:text-[var(--admin-ink)]",
                )}
                aria-hidden="true"
              />
              {collapsed && badge > 0 ? (
