...
export function ProjectCard({ project, featured = false, reverse = false, large = false }: ProjectCardProps) {
  ...
  return (
    <article className={cn("group", split && "lg:grid lg:grid-cols-2 lg:items-center lg:gap-12")}>
      ...
      <div className={cn("mt-6", split && "lg:mt-0", reverse && !large && "lg:order-1", large && "mx-auto max-w-3xl text-center lg:mt-10")}>
        <div
          className={cn(
            "flex flex-wrap items-center gap-x-3 gap-y-2 text-xs font-heading font-semibold uppercase tracking-[0.14em]",
        ...
        <p
          className={cn(
            "mt-3 text-muted",
            large ? "mx-auto max-w-2xl text-base md:text-lg" : "max-w-lg",
            !large && (split ? "text-base" : "text-sm"),
          )}
        >
          {project.summary}
        </p>
